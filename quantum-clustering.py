...
from numpy import array, zeros, empty, genfromtxt
...
class Model(object):
	...
	def compute_normal_const(self):
		...
		for col in range(self.m): 
			total = 0.0
			for i in range(self.n): total += self.x[i][col]
			total /= float(self.n)
			self.normal_const.append(total)

	def normalize(self):
		for col in range(self.m): self.x[:, col] /= self.normal_const[col]

	def compute_phi(self, x0):
		phi = 0
		for i in range(self.n): phi += exp(-(euclidean(x0, self.x[i])**2)/2/self.delta/self.delta)
		return phi
	...
	def compute_d(self): # can only compute half
		self.d = []
		for i in range(self.n):
			self.d.append([])
			for j in range(self.n):
				self.d[i].append(0)
				for k in range(self.m): self.d[i][j] += self.same(self.x[i][k], self.x[j][k])
	...
	def compute_performance(self):
		...
		for i in range(self.n-1):
			for j in np.arange(i+1, self.n):
				...
				if result == 1 and answer == 1: score += 1
				elif result == 1 and answer == 0: pscore += 1
				elif result == 0 and answer == 1: rscore += 1
				else: nscore += 1
	...
	def pca(self, dim):
		...
		pca = PCA(n_components = dim, copy = False) # what is copy?
	...
	def pcaVisualization(self):
		...
		for i in range(self.id.shape[0]): colors_list.append(colors[int(self.id[i])-1])
		plt.scatter(self.x[:, 0], self.x[:, 1], c = colors_list, marker = 'o', s = 40)#, alpha = 0.8)
		plt.show()
	...
	def kmeans(self, num_c):
		...
		self.c = []
		for i in range(self.x.shape[0]): self.c.append(self.kmeans.predict([self.x[i]]))
	...
	def compute_potential(self):
		...
		for i in range(self.n): # point to get potential energy
			...
			for k in range(self.n): # charge point
				sigma += (euclidean(self.x[k], self.x[i])**2)*exp(-(euclidean(self.x[k], self.x[i])**2)/2.0/self.delta/self.delta)
	...
	def potential_visualization(self):
		...
		c_list = []
		for i in range(self.n):
			if self.v[i] < 0.2: c_list.append('greenyellow')
			elif self.v[i] < 0.4: c_list.append('yellow')
			elif self.v[i] < 0.55: c_list.append('gold')
			elif self.v[i] < 0.7: c_list.append('orange')
			elif self.v[i] < 0.85: c_list.append('r')
			else: c_list.append('b')
	...
	def build_space(self, num):
		...
		for i in range(num): # point to get potential energy
			for j in range(num):
				sigma = 0
				for k in range(self.n): # charge point
					sigma += (euclidean(self.x[k], y[i*num+j])**2)*exp(-(euclidean(self.x[k], y[i*num+j])**2)/2.0/self.delta/self.delta)
		...
		c_list = []
		for i in range(num):
			if v[i] < 0.2: c_list.append('greenyellow')
			elif v[i] < 0.4: c_list.append('yellow')
			elif v[i] < 0.55: c_list.append('gold')
			elif v[i] < 0.7: c_list.append('orange')
			elif v[i] < 0.85: c_list.append('r')
			else: c_list.append('b')
	...
	def addToCluster(self, cl):
		self.c = []
		for i in range(self.x.shape[0]): self.c.append(0)
	...
	def qc(self):
		...
		v = self.normalize2(v)
		time = 0
		c = [] # 2 dim, 1 for cluster no, 1 for members in it
		...
		while x.shape[0]!=0:
			...
			for i in range(x.shape[0]):
				if self.d[index][i] <= self.beta: c[time-1].append(i)
			length = len(c[time-1])
	...
	def visual(self):
		...
		'#C44E52', '#6d904f', '#FFC400', 'm', 'g', 'k'] # greenyellow, 
		print "Visualization shape = ", self.x.shape
		
		for i in range(self.x.shape[0]): colors_list.append(colors[self.c[i]])
		plt.scatter(self.x[:, 0], self.x[:, 1], c = colors_list, marker = 'o', s = 40)#, alpha = 0.8)
		plt.show()
